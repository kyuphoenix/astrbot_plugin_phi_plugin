...
class AstrBotPhiPlugin(Star):
    ...
    async def phi_arcgros(self, event: AstrMessageEvent):
        ...
        async for result in self._dispatch_phi_command(event, self._extract_group_command(event.get_message_str()) or 'arcgros'):
            yield result
    ...
    async def phi_auth(self, event: AstrMessageEvent):
        ...
        async for result in self._dispatch_phi_command(event, 'auth'):
            yield result
    ...
    async def phi_b30(self, event: AstrMessageEvent):
        ...
        async for result in self._dispatch_phi_command(event, self._extract_group_command(event.get_message_str()) or 'b30'):
            yield result

    @phi.command('best', desc="生成文字版 B30 成绩列表。")
    async def phi_best(self, event: AstrMessageEvent):
        """生成文字版 B30 成绩列表。"""
        async for result in self._dispatch_phi_command(event, 'best'):
            yield result
    ...
    async def phi_down(self, event: AstrMessageEvent):
        """下载或更新插件资源、曲绘资源。"""
        async for result in self._dispatch_phi_command(event, 'down'):
            yield result
    ...
    async def phi_x30(self, event: AstrMessageEvent):
        ...
        async for result in self._dispatch_phi_command(event, self._extract_group_command(event.get_message_str()) or 'x30'):
            yield result
    ...
    async def phi_game_reply_listener(self, event: AstrMessageEvent):
        ...
        if result is None:
            return
        response = await self._send_command_result(event, result)
