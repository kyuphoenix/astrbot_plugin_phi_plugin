...
| `/p30`, `/x30`, `/fc30`, `/pN`, `/xN`, `/fcN` | `phi p30`, `phi x30`, `phi fc30`, `phi pN`, `phi xN`, `phi fcN` | Partial | `html/b19/dss2.art`, `html/b19/dss2.css` | Image chain uses original-style `dss2` resources, and dynamic aliases from `p1/x1/fc1` to `p100/x100/fc100` now route through the same command scripts with the requested count. User-specified background still needs parity. |
...
| `/achievement`, `/ahv` | `phi achievement`, `phi ahv` | Partial | Reuses `html/list/list.css` style output | Image mode now renders an original-resource list-style achievement panel, and `-v` now reuses the historical `oldInfo/<version>/change.csv` table data like upstream. Still needs exact original achievement/table wording and image layout parity. |
| `/list` | `phi list` | Partial | `html/list/list.art`, `list.css` | Image mode now uses original list resources, respects a configurable `list_score_max_num` guard like upstream, and renders original-style request lines for difficulty/ACC. Still needs exact upstream filter grammar and full list row/data parity. |
...
| `/info`, `/info1`, `/info2` | `phi info`, `phi info1`, `phi info2` | Partial | `html/userinfo/userinfo.art`, `userinfo.css`, `userinfo-old.css` | Image chain uses original resources, now parses a user-specified background song and routes `info1` to the current layout plus `info2` to the old layout. Still needs closer upstream old-template structure and broader live data parity checks. |
...
| Upstream command | Current AstrBot command | Status | Upstream template/resource | Notes / next action |
|---|---|---:|---|---|
| `/song` | `phi song` | Partial | `html/atlas/atlas.art`, `atlas.css` | Image mode now uses original atlas resources for song details with base64 illustrations and supports the upstream `-comment` option with an atlas comment panel. Still needs exact comments paging and chart/note-field parity. |
...
| `/table` | `phi table` | Partial | `html/table/table.art`, `table.css` | Image mode now uses original table resources, can overlay local player scores when a save is cached, and `-v` now reads historical `oldInfo/<version>/change.csv` data instead of the current catalog. Still needs exact upstream wording/error parity and broader version-table checks. |
