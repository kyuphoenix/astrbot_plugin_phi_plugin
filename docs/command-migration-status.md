...
| `/update` | `phi update` | Partial | `html/update/update.art`, `update.css` | Image mode now renders through converted Jinja2 `update/update` with centralized `update_data`, preserving the existing sync/progress logic while passing progress timeline, RKS graph, Notes/data delta, and first-record/task rows into the original template. Still needs exact upstream task source and live history edge-case comparison. |
