...
async def handle(ctx: CommandContext, user_id: str, args: str) -> CommandResult:
    ...
    if text and text not in {"ill", "曲绘", "original_ill", "illustration", "illustrations"}:
        return CommandResult.text("格式：phi down resources / phi down ill / phi down all\n用途：下载或更新原版 html、info、otherill 与曲绘资源。")
...
async def _download_resources(ctx: CommandContext) -> CommandResult:
    try:
        ...
        return CommandResult.text(f"原版资源下载/更新失败：{exc}")
    return CommandResult.text(
        f"原版资源{result.action}完成。\n"
        f"版本: {result.commit}\n"
        f"位置: {result.target}\n"
